...
- **New**: `career-advisor` subagent (`.claude/agents/career-advisor.md`, mirrored under `.agents/` only — Gemini CLI is deprecated and no longer available, and in any case cannot host the structurally-independent evidence-verifier dependency; see design.md) — scores HIGH_FIT/FIT jobs from `data/job_evaluations.json` against a second, positioning-specific weighted rubric (role/title level, dual-threat fit, domain fit, comp signal, technology fit), and drafts a positioning rationale plus resume bullet diff requests.
