...
2. **Evaluate job fit via interactive agent session**:
   1. Export unevaluated jobs without calling any external API:
      ...
      ```
   2. Invoke the `job-evaluator` subagent (pinned `model: haiku`) in a single batched call, passing it the exported jobs and `private/profile.md`. It scores every job against the fixed 5-dimension rubric and returns a JSON array of evaluation records tagged `"model": "claude-agent-session"`.
   3. Write the subagent's JSON array output to a scratch file (e.g. `private/.tmp_agent_evals.json`), then merge it back into `private/inbox_queue.json` and `private/job_evaluations.json`:
      ```bash
