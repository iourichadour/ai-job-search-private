...
- **`/upskill`** — aggregate mode: analyses all jobs in `private/job_search_tracker.csv`
...
### Aggregate mode
1. Read `private/job_search_tracker.csv`. Extract all rows. The columns are:
...
3. **Targeted mode ignores the tracker.** In targeted mode, analyse only the fetched posting. Do not load or reference `private/job_search_tracker.csv`.
