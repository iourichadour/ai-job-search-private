...
1. **Inbox Scanning & Extraction**:
   ...
   - Posting URLs are extracted and fetched using Playwright, saving raw text to `private/inbox_queue.json`.
...
2. **Job Fit Evaluation**:
   - **Default (interactive-agent round trip, no API key required)**: export unevaluated jobs with `python tools/evaluate_jobs_gemini.py --days 14 --filter-only`.
     ...
     - Write evaluations to scratch file `private/.tmp_agent_evals.json`, then merge back with `python tools/evaluate_jobs_gemini.py --save-evaluations private/.tmp_agent_evals.json`.
   ...
   - Both paths save structured ratings to `private/job_evaluations.json`.

3. **Application Drafting**:
   - When asked to `/apply`, run the drafting workflow to create tailored application artifacts in `private/applications/YYYY-MM_Company/`.
